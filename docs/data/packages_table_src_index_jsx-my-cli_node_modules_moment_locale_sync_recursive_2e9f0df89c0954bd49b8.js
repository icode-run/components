(self["webpackChunkicode_run"] = self["webpackChunkicode_run"] || []).push([["packages_table_src_index_jsx-my-cli_node_modules_moment_locale_sync_recursive_"],{
...
var style = __webpack_require__(85176);
...
;// CONCATENATED MODULE: ./packages/table/src/index.module.less
// extracted by mini-css-extract-plugin
/* harmony default export */ const index_module = ({"box":"packages-table-src-index-module__box","carouselBox":"packages-table-src-index-module__carouselBox"});
// EXTERNAL MODULE: ./packages/utils/es/dataType.js
var dataType = __webpack_require__(61204);
;// CONCATENATED MODULE: ./packages/table/src/index.jsx
 ...
 * Table
 * @author yanrong.tian
 * @date 2021/10/6 下午1:22:11
 */var Column=table/* default.Column */.Z.Column,ColumnGroup=table/* default.ColumnGroup */.Z.ColumnGroup;var orderTypes={'ascend':'ASC','descend':'DESC','ASC':'ascend','DESC':'descend'};var MyTable=/*#__PURE__*/function(_Component){_inherits(MyTable,_Component);var _super=_createSuper(MyTable);function MyTable(){var _this;_classCallCheck(this,MyTable);for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}_this=_super.call.apply(_super,[this].concat(args));_this.state={current:1};_this.topTitleChildren={};_this.prevSorter='';_this.onChange=function(pagination,filters,sorter,extra){var columns=_this.state.columns;var _this$props=_this.props,onFetch=_this$props.onFetch,orderType=_this$props.orderType,orderKey=_this$props.orderKey,_this$props$order=_this$props.order,order=_this$props$order===void 0?{}:_this$props$order;if(extra.action==='sort'){// if(orderType && typeof orderType.onChange === 'function') {
//   const orderTypes = {
//     'ascend': 'ASC',
//     'descend': 'DESC'
//   }
//   orderType.onChange(orderTypes[sorter.order] || '');
// }
// if(orderKey && typeof orderKey.onChange === 'function') {
//   const field = sorter.column ? sorter.field : '';
//   orderKey.onChange(field);
// }
if(order.onChange&&typeof order.onChange==='function'){var outputType=order.outputType||'[orderKey, orderType]';var _orderType=orderTypes[sorter.order]||'';var _orderKey=sorter.column?sorter.field:'';var value=[];if(outputType==='[orderKey, orderType]'){value=[_orderKey,_orderType];}else if(outputType==='[[orderKey, orderType]]'){var isArray=order.value&&order.value[0]&&order.value[0][1]&&['ASC','DESC'].indexOf(order.value[0][1])>=0;var oldValue=isArray?order.value:[];oldValue.forEach(function(item){if(item&&item[0]!==_orderKey&&item[1]){value.push(item);}});value=[[_orderKey,_orderType]].concat(value);}order.onChange(value);}}};return _this;}_createClass(MyTable,[{key:"componentDidMount",value:function componentDidMount(){// const {data} = this.props;
// this.setState({
//   data
// })
}},{key:"componentDidUpdate",value:function componentDidUpdate(){}},{key:"render",value:function render(){var _this2=this;var _this$props2=this.props,className=_this$props2.className,size=_this$props2.size,bordered=_this$props2.bordered,isLoading=_this$props2.isLoading,_this$props2$isPagina=_this$props2.isPagination,isPagination=_this$props2$isPagina===void 0?false:_this$props2$isPagina,data=_this$props2.data,_this$props2$limit=_this$props2.limit,limit=_this$props2$limit===void 0?{}:_this$props2$limit,_this$props2$offset=_this$props2.offset,offset=_this$props2$offset===void 0?{}:_this$props2$offset,dataCount=_this$props2.dataCount,simple=_this$props2.simple,showQuickJumper=_this$props2.showQuickJumper,rowSelection=_this$props2.rowSelection,isRowSelection=_this$props2.isRowSelection,onFetch=_this$props2.onFetch;var _this$state$columns=this.state.columns,columns=_this$state$columns===void 0?[]:_this$state$columns;this.topTitleChildren={};var pageSize=parseInt(limit.value,10)||10;var current=(parseInt(offset.value,10)||0)/pageSize+1;if(rowSelection){rowSelection.selectedRowKeys=[];}return/*#__PURE__*/index_js_default().createElement("div",{className:"".concat(index_module.box," ").concat(className),style:this.props.style},/*#__PURE__*/index_js_default().createElement(table/* default */.Z,_extends({loading:isLoading,bordered:bordered,size:size,dataSource:data,onChange:this.onChange},isRowSelection?{rowSelection:{selectedRowKeys:[],onChange:function onChange(values){}}}:{},{pagination:isPagination?{current:current||1,pageSize:pageSize,total:dataCount,simple:simple,size:size,showQuickJumper:showQuickJumper,onChange:function onChange(page,pageSize){if(offset.onChange&&typeof offset.onChange==='function'){offset.onChange((page-1)*pageSize);}}}:false}),columns.map(function(item,index){var columnProps=_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({title:item.title,key:item.key,render:item.render||null},!item.render?{dataIndex:item.dataIndex||null}:{}),{},{sorter:item.sorter||false,sortOrder:item.sortOrder||false},item.width?{width:item.width}:{}),item.align?{align:item.align}:{}),item.fixed?{fixed:item.fixed}:{}),item.ellipsis?{ellipsis:item.ellipsis}:{});if(item.topTitle){_this2.topTitleChildren[item.topTitle]=_this2.topTitleChildren[item.topTitle]||[];_this2.topTitleChildren[item.topTitle].push(/*#__PURE__*/index_js_default().createElement(Column,columnProps));var topTitleChildrenLength=_this2.topTitleChildren[item.topTitle].length;if(topTitleChildrenLength>=2&&columns[index+1]&&columns[index+1].topTitle!==item.topTitle){return/*#__PURE__*/index_js_default().createElement(ColumnGroup,{title:item.topTitle},_this2.topTitleChildren[item.topTitle]);}else{return null;}}else{return/*#__PURE__*/index_js_default().createElement(Column,columnProps);}})));}}],[{key:"getDerivedStateFromProps",value:function getDerivedStateFromProps(nextProps,prevState){var columns=nextProps.columns,_nextProps$order=nextProps.order,order=_nextProps$order===void 0?{}:_nextProps$order;var propsColumnsString=JSON.stringify({columns:columns,order:order.value// orderType: orderType.value,
// orderKey: orderKey.value
}||[]);//if (propsColumnsString !== prevState.propsColumnsString) {
var orderValue=(0,dataType/* getOutputValue */.jG)({outputType:'[orderKey, orderType]',value:order.value});var orderKey=orderValue&&orderValue[0]?orderValue[0]:'';var orderType=orderValue&&orderValue[1]?orderValue[1]:'';var newColumns=(columns||[]).map(function(item,index){var options=_objectSpread({},item);if(item.isSorter){options.sorter=function(a,b){return a[item.keyName]-b[item.keyName];};if(orderKey&&orderType&&orderKey===item.keyName){options.sortOrder=orderTypes[orderType];}}return _objectSpread(_objectSpread({},options),{},{title:item.title,dataIndex:item.keyName,key:"".concat(item.keyName,"_").concat(index)});});return{propsColumnsString:propsColumnsString,columns:newColumns};// }
//return null;
}}]);return MyTable;}(index_js_.Component);/* harmony default export */ const src = (MyTable);
